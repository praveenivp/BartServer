...
def picsGadget(connection):
   logging.info("Python reconstruction running - reading readout data")
   h=connection.header
   Bartcmds=(h.userParameters.userParameterString[0].value)
   ecalibcmd= Bartcmds.split(';')[0]
   picscmd= Bartcmds.split(';')[1]
   start = time.time()
   counter = 0
   for acquisition in connection:
       for reconBit in acquisition:
           try:
               print("calling BART : "+ ecalibcmd +"\n")
               refdata=np.array(reconBit.data.data)
               sens = bart(1, ecalibcmd,  refdata)
               print("calling BART : "+ picscmd +"\n")
               pics_im = bart(1, picscmd,refdata,sens)
               reference_header=reconBit.data.headers.flat[34]
               send_reconstructed_images(connection,pics_im,reference_header)
